...
- Refactored Node.js-specific code out of `cache.ts` and `client.ts` into dedicated helper modules (`node-cache-io.ts`, `node-env.ts`); bundlers replace only these helpers with browser-safe no-op stubs via the `browser` field in `package.json` (Option B), keeping `ResponseCache` and `KakenApiClient` as single-source files
- Updated `package.json` `browser` field: maps `dist/node-cache-io.js` → `dist/node-cache-io.browser.js` and `dist/node-env.js` → `dist/node-env.browser.js`

### Removed

- `src/cache.browser.ts` — no longer needed; Node.js I/O is now isolated in `node-cache-io.ts`
...
[Unreleased]: https://github.com/hirakinii/kaken-api-client-typescript/compare/v0.1.2...HEAD
[0.1.2]: https://github.com/hirakinii/kaken-api-client-typescript/compare/v0.1.1...v0.1.2
